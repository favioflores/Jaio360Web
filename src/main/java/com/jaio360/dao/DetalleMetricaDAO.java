...
public class DetalleMetricaDAO implements Serializable {
    ...
    public void eliminaDetalleMetricaPosterior(Metrica metrica) throws HibernateException {
        try {
            iniciaOperacion();
    ...
    public DetalleMetrica obtenDetalleMetrica(int idProyecto, int idNuOrden) throws HibernateException {

        try {

            iniciaOperacion();
            Query sql = sesion.createQuery("from DetalleMetrica dm where dm.metrica.proyecto.poIdProyectoPk = :idProyecto and dm.deNuOrden = :idNuOrden ");
            sql.setInteger("idProyecto", idProyecto);
            sql.setInteger("idNuOrden", idNuOrden);

            Object obj = sql.uniqueResult();

            if (obj != null) {
                return (DetalleMetrica) obj;
            } else {
                return null;
            }
