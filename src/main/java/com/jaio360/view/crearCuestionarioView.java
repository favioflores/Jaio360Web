...
public class crearCuestionarioView implements Serializable{
    ...
    private static Log log = LogFactory.getLog(crearCuestionarioView.class);
    ...
    public void loadRango(){
        this.lstDetalleMetrica = new ArrayList<>();
        ...
        if(metrica != null){
           ...
           for (DetalleMetrica varDetalleMetrica : lstTmpDetalleMetrica) {
               this.lstDetalleMetrica.add(varDetalleMetrica);
           } 
           this.intRangos = metrica.getMeNuRango();
           if(lstDetalleMetrica.size()<2){
               this.intRangoMin = 2;
           }else{
               this.intRangoMin = lstDetalleMetrica.size();
           }   
        } else{
    ...
    public void addMetrica(){
        if(lstDetalleMetrica.size()<this.intRangos){
        
            DetalleMetrica objDetalleMetrica = new DetalleMetrica();
            objDetalleMetrica.setDeNuOrden(this.lstDetalleMetrica.size());
            ...
            lstDetalleMetrica.add(objDetalleMetrica);

            this.strRangoSelec = "";

            FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO,"Agregar metrica","La metrica se agregó correctamente" );
            FacesContext.getCurrentInstance().addMessage(null, msg);
        }else{
            FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR,"Agregar metrica","No puede haber mas metricas que rango" );
            FacesContext.getCurrentInstance().addMessage(null, msg);
        }
    ...
    public void guardarEscala(){
        try{
            ...
            FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_FATAL,"Grabar metrica","Ocurrio un error al grabar los datos" );
            FacesContext.getCurrentInstance().addMessage(null, msg);
        }
    }
    ...
    public void eliminarCuestionario(Integer intIdCuestionario){
        boolean correcto = cuestionarioDAO.eliminaCuestionarioTotal(intIdCuestionario);
